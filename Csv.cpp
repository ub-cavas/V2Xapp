...
string csvConvertVehDataToString(Mri::VehData _veh) {
	...
	tekst	<< GetTimestamp() << ',' << "Mri_TrafficVeh" << ',' << _veh.color <<','<< _veh.lane_index << ',' << _veh.leading_vehicle_id
			...
			<< ',' << _veh.speed << ',' << _veh.timestamp << ',' << _veh.trailing_vehicle_id << ',' << _veh.turning_indicator << ',' << _veh.vehicle_id << ',' << _veh.vehicle_type << '\n';
...
void csvWriteFile( int indx_buff_to_save, long number_of_samples) {
	...
	ofstream csvFile;
	cout << "Start writing file at: " << GetTimestamp() << endl;
